...
export async function POST(request: Request) {
    ...
    try {
        ...
        for (const [index, q] of questions.entries()) {
            const time = Number(q.completionTime) || timePerQuestion;
            if (time <= 0) {
        ...
        const inputQuestions = questions.map((question, index) => ({
            questionIndex: index,
            completionTime: Number(question.completionTime) || timePerQuestion,
        }))

        // Insert the document into the 'timers' collection
        const result = await db.collection("timers").insertOne({
            examId,
            chainId,
